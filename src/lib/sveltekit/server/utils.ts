...
export function logVerbose(message: string, verbose: boolean = false) {
	if (verbose) {
		console.debug(`[verbose] ${new Date().toISOString()} [ConvexAuthSvelteKit] ${message}`);
	}
}
...
export function getConvexUrl(
	...
	if (!url && typeof process !== 'undefined' && process.env && process.env.PUBLIC_CONVEX_URL) {
...
export function validateDeploymentUrl(deploymentUrl: string) {
	...
	if (typeof deploymentUrl !== 'string') {
		throw new Error(`Invalid deployment address: found ${deploymentUrl as any}".`);
	}
