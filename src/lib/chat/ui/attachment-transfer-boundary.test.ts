...
const coordinator = readFileSync(resolve('src/lib/chat/ui/chat-context.svelte.ts'), 'utf8');
...
describe('AttachmentTransfer ownership boundary', () => {
	it('removes attempt registries and runUpload from ChatUIContext', () => {
		expect(coordinator).toContain('new AttachmentTransfer({');
		expect(coordinator).toContain('private readonly transfers');
		expect(coordinator).not.toContain('uploadAborters');
		expect(coordinator).not.toContain('retryJobs');
		expect(coordinator).not.toContain('private async runUpload');
	});
