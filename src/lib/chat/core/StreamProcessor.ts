...
/**
 * Process messages with streaming data
 *
 * Merges base messages with streaming deltas and caches to produce
 * display-ready messages with proper fallbacks.
 */
export function processMessagesWithStreaming(
	messages: ChatMessage[],
	streamMessages: Array<{ order: number; status: StreamStatus; streamId: string }>,
	streamingUIMessages: UIMessage[],
	cacheManager: StreamCacheManager
): DisplayMessage[] {
	// Build maps from streaming data
	const streamingTextMap = new Map<number, string>();
	const streamingReasoningMap = new Map<number, string>();
	const streamingStatusMap = new Map<number, StreamStatus>();
	...
	streamMessages.forEach((streamMsg) => {
		...
		cacheManager.updateStatusCache(streamMsg.order, streamMsg.status);
	});

	// Map UIMessages to extract text and reasoning
	streamingUIMessages.forEach((uiMsg) => {
		streamingTextMap.set(uiMsg.order, uiMsg.text || '');
		const reasoning = extractReasoning(uiMsg.parts as MessagePart[]);
		if (reasoning) {
			streamingReasoningMap.set(uiMsg.order, reasoning);
			cacheManager.updateReasoningCache(uiMsg.order, reasoning);
		}
	});

	// Process each message
	return messages.map((msg): DisplayMessage => {
		const isUser = msg.role === 'user';

		// Get streaming data for this message
		...
		const hasReasoningStream = streamStatus !== undefined || cacheManager.hasStatusCache(msg.order);
...
/**
 * Process messages without streaming data (fallback for query reloads)
 */
export function processMessagesWithoutStreaming(
	messages: ChatMessage[],
	cacheManager: StreamCacheManager
): DisplayMessage[] {
	return messages.map((msg): DisplayMessage => {
		...
		return {
			...
			hasReasoningStream: cacheManager.hasStatusCache(msg.order)
		};
	});
}
