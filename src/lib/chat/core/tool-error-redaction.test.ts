...
describe('tool error redaction', () => {
	...
	it('leaves successful output data with an error field unchanged', () => {
		const source = messageDoc([
			{
				type: 'tool-result',
				toolCallId: 'call-1',
				toolName: 'lookup',
				output: { type: 'json', value: { error: 'domain value', ok: true } }
			}
		]);
	...
	])('redacts the $part.type stream marker', ({ part, field }) => {
	...
	it('keeps tool failures out of the next model prompt and UI stream', async () => {
		...
		expect(JSON.stringify(chunks)).toContain(SAFE_TOOL_ERROR_MESSAGE);
	});
