...
describe('first-party boundary compile-time contracts', () => {
	...
	it('carries query envelopes and mutation results without default-any function references', () => {
		...
		expectTypeOf<FunctionReturnType<ChatMessagesQuery>>().toEqualTypeOf<MessagesQueryResponse>();
		expectTypeOf<FunctionReturnType<ChatMessagesQuery>['page'][number]>().not.toBeAny();
