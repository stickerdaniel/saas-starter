	...
	>
		{#each items as sub (sub.id)}
			<Sidebar.MenuSubItem>
				...
				{#if sub.timestamp}
					<span
						class="pointer-events-none absolute inset-y-0 -right-2 flex items-center justify-end overflow-hidden rounded-r-md bg-sidebar-accent-hover thread-timestamp-mask pr-3 pl-5 text-xs whitespace-nowrap text-muted-foreground opacity-0 group-hover/menu-sub-item:opacity-100 group-active/menu-sub-item:translate-y-px group-has-data-[active=true]/menu-sub-item:bg-sidebar-accent"
					>
