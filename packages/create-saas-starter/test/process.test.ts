...
describe('process contract', () => {
	...
	it('treats an exactly empty PATH as the original CLI cwd', async () => {
		const originalCwd = await mkdtemp(path.join(tmpdir(), 'create-saas-starter-empty-path-only-'));
		temporaryDirectories.push(originalCwd);
		const executableName = process.platform === 'win32' ? 'bun.exe' : 'bun';
		...
		await expect(
			resolveBunExecutable({ PATH: '' }, originalCwd, process.platform, null)
