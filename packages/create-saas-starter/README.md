...
The package requires Node.js 22.16.0 or newer. Setup and installation require Bun 1.3.9 or newer. Archive writes and template commands run in a protected per-user staging directory on the target filesystem. The completed project is published through an exclusive final claim, so replacing the requested target cannot redirect setup execution. Windows staging stays inside the current user profile; POSIX staging requires root/current-user ownership and sticky protection for shared writable ancestors.
