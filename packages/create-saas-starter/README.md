...
The package requires Node.js 22.16.0 or newer. Setup and installation require Bun 1.3.9 or newer. Project creation currently requires a POSIX platform: Node.js does not expose enough Windows ACL information to prove that another local user cannot replace a claimed target before its setup code runs. Read-only commands such as `--help`, `--version`, and `--dry-run` remain available on Windows.
