...
describe('deployment pipeline', () => {
	...
	it('preserves the entire preview step order, URLs, key selection and build secret stripping', async () => {
		...
		expect(h.events).toEqual([
			...
			'bunx convex env set --deployment-name preview-backend CAPABILITY_PROFILE preview',
	...
	it('preserves development/unknown-platform branching and optional translation sync', async () => {
		const development = harness({
			...
			VERCEL_URL: 'development.example.test',
		...
		const unknown = harness({ SITE_URL: 'https://production.example.test' });
	...
	it('keeps the successful CLI exit code at zero and does not report an error', async () => {
		const saved = process.exitCode ?? 0;
		try {
			process.exitCode = 0;
			const h = harness();
