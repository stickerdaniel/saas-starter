...
if [ "$VERCEL_ENV" = "production" ]; then
  ...
  bun scripts/validate-convex-env.ts --prod
else
  ...
  if [ -n "$VERCEL_URL" ] && [ -n "$VERCEL_GIT_COMMIT_REF" ]; then
    ...
    bunx convex env set SITE_URL "$PREVIEW_SITE_URL" --preview-name "$VERCEL_GIT_COMMIT_REF" 2>/dev/null || {
...
# Extract deployment URL from CONVEX_DEPLOY_KEY (format: prod:name|token or preview:team:project:name|token)
# Production: prod:keen-labrador-829|... -> keen-labrador-829
# Preview: preview:team:project:name|... -> name
DEPLOYMENT_NAME=$(echo "$CONVEX_DEPLOY_KEY" | cut -d'|' -f1 | rev | cut -d':' -f1 | rev)
export PUBLIC_CONVEX_URL="https://${DEPLOYMENT_NAME}.convex.cloud"
export PUBLIC_CONVEX_SITE_URL="https://${DEPLOYMENT_NAME}.convex.site"
