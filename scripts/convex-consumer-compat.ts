 ...
 */
function surfaceAt(commit: string): Surface {
...
const promised = surfaceAt(baseline.commit);
...
const required = consumerReferencesAt(baseline.commit).filter(
	(ref) => promised.get(ref.identifier)?.visibility === ref.visibility
);
const available = surfaceOf(CONVEX_ROOT);
